...
class Cities extends CI_Controller {
	...
	public function index( $msg = NULL)
	{
        ...
        if(! $this->session->userdata('userrole')){
			...
			$this->load->view('superadmin/cities/index', $data);
	...
	public function add($id = NULL){
        ...
        $data['countries'] = $this->country_model->get_all_countries();
			...
			if ($this->form_validation->run() === FALSE)
			{
				...
				$this->load->view('superadmin/templates/sidebar', $data);
				$this->load->view('superadmin/cities/add', $data);
				$this->load->view('superadmin/templates/footer');
			}
			else
			{
                    if($this->cities_model->set_city()){
    ...
    public function createThumbnail($filename)
    {
        ...
        $data['city'] = $this->cities_model->get_all_cities($id);
            $data['countries'] = $this->country_model->get_all_countries();
			$this->load->helper('form');
			$this->load->library('form_validation');
            $this->form_validation->set_rules('name', 'Name', 'required|min_length[3]');
			...
			if ($this->form_validation->run() === FALSE)
			{
				$this->load->view('superadmin/templates/header');
				$this->load->view('superadmin/templates/sidebar', $data);
				$this->load->view('superadmin/cities/edit', $data);
				$this->load->view('superadmin/templates/footer');
			}
			else
			{
                    $this->cities_model->update_city();
                    $this->session->set_flashdata('msg', 'City Information is updated Successfully!');
                    redirect('cities');
			}
    ...
    public function history($patientid){
     ...
     public function delete($id = NULL){
			...
			if($this->cities_model->delete_city($id)){
               
				$this->session->set_flashdata('msg', 'City is deleted Successfully!');
				redirect('cities');
			}
